...
The dashboard includes a **Marketing OS** tab that reimplements the useful working surface (not a clone of another repo):

- **Brand context** — save/load product, audience, positioning, proof, voice, and constraints. Generators read this file.
- **Hook engine** — 18-tactic matrix with original copy and heuristic scores (0–100). Missing proof is tagged `[NEED: figure]`; numbers are never invented.
- **Launch / social packet** — story, timeline, LinkedIn / X / Instagram artifacts, email, FAQ, and an explicit gaps list.

### Optional SocialClaw poster hook

SocialClaw is a hosted/paid service. This app does **not** ship keys and will not fake a publish.

Set `SOCIALCLAW_API_URL` on the host if you have a workspace. Until that env is present, the UI stays in a **not configured** state and the preview button stays disabled.

If the process also has `SC_API_KEY` (SocialClaw's own env name), it is sent as `Authorization: Bearer`. Do not put a invented key in settings.
